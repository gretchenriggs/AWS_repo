''' Running CNN to classify satellite images as containing only natural
    ...
    (53600, 124, 124, 1) for the X images below
'''
import cPickle
import resource
import sys
...
def preproc(X_train, X_test):
    ...
    '''
    # Converting RGB to Gray Scale Images to train for Moon/Mars prediction test
    # The pixel values after converting to Gray Scale are between 0 & 1
    X_train_gray = np.empty([0, 124, 124], dtype="float32")
    X_test_gray = np.empty([0, 124, 124], dtype="float32")
    for i in xrange(len(X_train)):
        X_train_gray = np.insert(X_train_gray, 0, rgb2gray(X_train[i]), axis=0)
    for i in xrange(len(X_test)):
        X_test_gray = np.insert(X_test_gray, 0, rgb2gray(X_test[i]), axis=0)
    ...
    #   Test set
    mean_X_train_gray = np.mean(X_train_gray)
    for i in range(len(X_train_gray)):
        X_train_gray[i] = X_train_gray[i] - mean_X_train_gray
    for i in range(len(X_test_gray)):
        X_test_gray[i] = X_test_gray[i] - mean_X_train_gray
...
def train_test(X, y, nb_classes, test_percent=0.20):
    ...
    # Calling function to set pixels to Gray Scale with values between 0-1
    #  and center around zero.
    X_train_gray, X_test_gray = preproc(X_train, X_test)

    # Convert class vectors to binary class matrices
    y_train = np_utils.to_categorical(y_train, nb_classes)
    y_test = np_utils.to_categorical(y_test, nb_classes)

    return X_train_gray, X_test_gray, y_train, y_test


def keras_inp_prep(X_train_gray, X_test_gray, img_dep, img_rows, img_cols, \
                   dim_ordering='tf'):
    ''' Reshape feature matrices X_train_gray, X_test_gray to be compatible\
          with neural network (NN) expected input_shape.
        ...
        Input: X_train_gray, X_test_gray - arrays
               img_dep, img_rows, img_cols - integers
               dim_ordering - string
        Output: X_train_gray, X_test_gray - arrays
                input_shape - tuple
    '''
    if K.image_dim_ordering() == 'th':
        X_train_gray = X_train_gray.reshape(X_train_gray.shape[0], img_dep,\
                                            img_rows, img_cols)
        X_test_gray = X_test_gray.reshape(X_test_gray.shape[0], img_dep,\
                                          img_rows, img_cols)
        input_shape = (img_dep, img_rows, img_cols)
    else:
        X_train_gray = X_train_gray.reshape(X_train_gray.shape[0], img_rows,\
                                            img_cols, img_dep)
        X_test_gray = X_test_gray.reshape(X_test_gray.shape[0], img_rows,\
                                          img_cols, img_dep)
        input_shape = (img_rows, img_cols, img_dep)
    return X_train_gray, X_test_gray, input_shape


def cnn(X_train_gray, y_train, X_test_gray, y_test, kernel_size, pool_size,\
    ...
    ''' This is the Convolutional Neural Net architecture, which is the
        ...
        Input: X_train_gray, y_train (training data)- arrays
               X_test_gray, y_test (cross-validation data) - arrays
    ...
    model.add(Convolution2D(32, kernel_size, kernel_size, \
                            border_mode='valid', input_shape=X_train_gray.shape[1:]))
    ...
    model.fit(X_train_gray, y_train, batch_size=batch_size, nb_epoch=nb_epoch,
              verbose=1, validation_data=(X_test_gray, y_test))

    # Evaluating final accuracy on training and test sets
    score = model.evaluate(X_test_gray, y_test, verbose=0)
    print('Test accuracy:', score[1])
    score_train = model.evaluate(X_train_gray, y_train, verbose=0)
    print('Train accuracy:', score_train[1])

    return model, score, score_train


def model_performance(model, X_train_gray, X_test_gray, y_train, y_test):
    ...
    y_test_pred = model.predict_classes(X_test_gray)
    y_train_pred = model.predict_classes(X_train_gray)

    # Prediction's probability outputs for Test & Train datasets
    y_test_pred_proba = model.predict_proba(X_test_gray)
    y_train_pred_proba = model.predict_proba(X_train_gray)
...
if __name__ == '__main__':
    ...
    # Reshape X_train_gray & X_test_gray from
    #   Nx124x124 to Nx124x124x1, where N=number of images
    X_train_gray = np.reshape(X_train_gray, (X_train_gray.shape[0], \
                              X_train_gray.shape[1], X_train_gray.shape[2], 1))
    X_test_gray = np.reshape(X_test_gray, (X_test_gray.shape[0], \
                              X_test_gray.shape[1], X_test_gray.shape[2], 1))
    ...
    X_train_gray, X_test_gray, input_shape = keras_inp_prep(X_train_gray,\
                X_test_gray, img_dep, img_rows, img_cols, dim_ordering='tf')

    # Run Convolutional Neural Net
    model, score, score_train = cnn(X_train_gray, y_train, X_test_gray, y_test,\
                        kernel_size, pool_size, batch_size, nb_classes,\
                        nb_epoch)

    # Save the model to disk
    model.save('finalized_model_all_gray.h5')

    # Evaluating CNN Model performance
    y_train_pred, y_test_pred, y_train_pred_proba, y_test_pred_proba, \
       conf_matrix = model_performance(model, X_train_gray, X_test_gray, y_train, y_test)
